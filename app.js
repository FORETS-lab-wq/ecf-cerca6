...
const SLBLS={
  ...
  sign_st_andre:'Priorité à droite',sign_sens_unique:'Sens unique',
...
function renderD(d){
  ...
  else if(d.t==='text'){ctx.font=`bold ${14+d.sz*2.5}px -apple-system,sans-serif`;ctx.fillStyle=d.color;ctx.textBaseline='top';ctx.fillText(d.txt,d.x,d.y);}
  else if(d.t==='step'){
...
function drawSpecial(el){
  ...
  else if(t==='sign_gira'){ctx.fillStyle='#0a7aff';ctx.strokeStyle='#fff';ctx.lineWidth=2.5;ctx.beginPath();ctx.arc(0,0,s*.42,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.strokeStyle='#fff';ctx.lineWidth=3.5;ctx.lineCap='round';ctx.beginPath();ctx.arc(0,0,s*.22,-Math.PI*.75,Math.PI*.75,false);ctx.stroke();ctx.fillStyle='#fff';const ex=s*.22*Math.cos(Math.PI*.75),ey=s*.22*Math.sin(Math.PI*.75);ctx.beginPath();ctx.moveTo(ex,ey);ctx.lineTo(ex-10,ey-10);ctx.lineTo(ex+8,ey-6);ctx.closePath();ctx.fill();}
...
function drawCarSchema(cx,cy){
  ...
  const scale=Math.min(W/620,H/320)*.85;
  ctx.save();ctx.translate(cx,cy+10);ctx.scale(scale,scale);

  // Sol
  ctx.fillStyle='#c8c0b0';ctx.fillRect(-280,90,560,12);
  ctx.strokeStyle='#aaa';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(-280,90);ctx.lineTo(280,90);ctx.stroke();

  // Carrosserie principale
  const carG=ctx.createLinearGradient(-220,-60,220,80);
  carG.addColorStop(0,'#a8c8e8');carG.addColorStop(.4,'#c4ddf0');carG.addColorStop(1,'#7aaec8');
  ctx.fillStyle=carG;ctx.strokeStyle='#5a8eb0';ctx.lineWidth=2.5;
  ctx.beginPath();
  ctx.moveTo(-220,80);ctx.lineTo(-240,30);ctx.lineTo(-180,-10);
  ctx.lineTo(-100,-55);ctx.lineTo(60,-55);ctx.lineTo(140,-10);
  ctx.lineTo(220,30);ctx.lineTo(220,80);ctx.closePath();
  ctx.fill();ctx.stroke();

  // Vitres
  ctx.fillStyle='rgba(200,230,255,.6)';ctx.strokeStyle='#5a8eb0';ctx.lineWidth=1.5;
  ctx.beginPath();ctx.moveTo(-170,-8);ctx.lineTo(-100,-50);ctx.lineTo(30,-50);ctx.lineTo(30,-5);ctx.closePath();ctx.fill();ctx.stroke();
  ctx.beginPath();ctx.moveTo(40,-5);ctx.lineTo(40,-50);ctx.lineTo(110,-50);ctx.lineTo(130,-8);ctx.closePath();ctx.fill();ctx.stroke();

  // Roue avant
  const drawWheel=(wx,wy)=>{
    ctx.fillStyle='#222';ctx.beginPath();ctx.arc(wx,wy,38,0,Math.PI*2);ctx.fill();
    const wg=ctx.createRadialGradient(wx-8,wy-8,4,wx,wy,28);
    wg.addColorStop(0,'#555');wg.addColorStop(1,'#111');
    ctx.fillStyle=wg;ctx.beginPath();ctx.arc(wx,wy,28,0,Math.PI*2);ctx.fill();
    ctx.fillStyle='#888';ctx.beginPath();ctx.arc(wx,wy,12,0,Math.PI*2);ctx.fill();
    ctx.strokeStyle='#555';ctx.lineWidth=2;
    for(let i=0;i<5;i++){const a=i*Math.PI*2/5;ctx.beginPath();ctx.moveTo(wx,wy);ctx.lineTo(wx+22*Math.cos(a),wy+22*Math.sin(a));ctx.stroke();}
  };
  drawWheel(-145,80);drawWheel(155,80);

  // Blocs colorés composants
  const comps=[
    {id:'mot',x:-155,y:20,w:72,h:52,col:'#e05c00',bg:'#fff4ee',label:'1\nMOTEUR'},
    {id:'emb',x:-68,y:28,w:42,h:36,col:'#0a7aff',bg:'#eef5ff',label:'2\nEMBR.'},
    {id:'bvt',x:10, y:20,w:72,h:52,col:'#30b94a',bg:'#eefff2',label:'3\nBOÎTE'},
    {id:'arb',x:88, y:38,w:58,h:18,col:'#f08000',bg:'#fff8ee',label:'4'},
  ];
  comps.forEach(c=>{
    ctx.fillStyle=c.bg;ctx.strokeStyle=c.col;ctx.lineWidth=2.5;
    rr(ctx,c.x-c.w/2,c.y-c.h/2,c.w,c.h,8);ctx.fill();ctx.stroke();
    ctx.fillStyle=c.col;ctx.font=`bold ${c.id==='arb'?9:10}px sans-serif`;ctx.textAlign='center';ctx.textBaseline='middle';
    c.label.split('\n').forEach((ln,i,arr)=>ctx.fillText(ln,c.x,c.y+(i-(arr.length-1)/2)*12));
  });

  // Arbre de transmission (ligne)
  ctx.strokeStyle='#f08000';ctx.lineWidth=5;ctx.lineCap='round';
  ctx.beginPath();ctx.moveTo(45,47);ctx.lineTo(115,47);ctx.stroke();
  // flèche
  ctx.fillStyle='#f08000';ctx.beginPath();ctx.moveTo(108,47);ctx.lineTo(96,41);ctx.lineTo(96,53);ctx.closePath();ctx.fill();

  // Roue arrière numéro 5
  ctx.fillStyle='#9b59b6';ctx.font='bold 10px sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';
  rr(ctx,130,62,28,22,6);
  ctx.fillStyle='rgba(155,89,182,.15)';rr(ctx,130,62,28,22,6);ctx.fill();
  ctx.strokeStyle='#9b59b6';ctx.lineWidth=2;rr(ctx,130,62,28,22,6);ctx.stroke();
  ctx.fillStyle='#9b59b6';ctx.fillText('5',130,73);
  ...
  legend.forEach((l,i)=>{
    const ix=lx+i*108;
    ctx.fillStyle=l.col;ctx.beginPath();ctx.arc(ix,ly,10,0,Math.PI*2);ctx.fill();
    ctx.fillStyle='#fff';ctx.font='bold 9px sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(l.n,ix,ly);
    ctx.fillStyle='#333';ctx.font='9px sans-serif';ctx.textAlign='left';ctx.textBaseline='middle';ctx.fillText(l.lbl,ix+14,ly);
  });

  // Titre
  ctx.fillStyle='#1a1a1c';ctx.font='bold 15px sans-serif';ctx.textAlign='center';ctx.textBaseline='bottom';
  ctx.fillText('Embrayage — chaîne cinématique',0,-72);
...
function loadScene(name){
  ...
  if(name==='voie_insertion'){
    // 2x2 voies + voie d'insertion + voie de décélération
    const y1=cy-80, y2=cy-26, y3=cy+26, y4=cy+80;
    // Accotements
    drawings.push({t:'rect',x:0,y:0,w:W,h:cy-100,color:'rgba(120,170,80,.25)',fill:'rgba(120,170,80,.25)',sz:0});
    drawings.push({t:'rect',x:0,y:cy+100,w:W,h:H-cy-100,color:'rgba(120,170,80,.25)',fill:'rgba(120,170,80,.25)',sz:0});
    // Voies principales 2x2
    ...
    // Voie d'insertion (en bas, commence au milieu)
    drawings.push({t:'roadcurve',x1:W*.35,y1:cy+140,x2:W,y2:y3,lanes:1});
    // Voie de décélération (en haut, finit au milieu)
    drawings.push({t:'roadcurve',x1:0,y1:y1,x2:W*.65,y2:cy-140,lanes:1});
    // Labels
    drawings.push({t:'text',x:W*.5,y:cy-140,text:'Voie de décélération',color:'#1c1c1e',sz:14});
    drawings.push({t:'text',x:W*.5,y:cy+148,text:'Voie d\'insertion',color:'#1c1c1e',sz:14});
    setTitle('2×2 voies – insertion/décélération');redraw();return;
  }
  if(name==='meca_vue'){
